...
},
{
  type:'input',
  name:'text',
  message:'[>] Komen yang disampaikan (gunakan [|] jika lebih dari 1):',
  validate: function(value){
    if(!value) return 'Gaboleh Kosong';
    return true;
  }
...
const CommentAndLike = async function(session, accountId, text){
  ...
  if (result.length > 0) {
    ...
    const printComment = Comment ? chalk`{green Comment}` : chalk`{red Fail Comment}`;
    const printLike = Like ? chalk`{green Like}` : chalk`{red Fail Like}`;
...
const Excute = async function(User, TargetUsername, Text, Sleep, mysyntx){
  try {
    ...
    console.log(chalk`{green  Sukses Login, }{yellow Mencoba mendapat ID dan Followers Target . . .}`)
    ...
    console.log(chalk`{cyan  [?] Mencoba Untuk menjalankan Tools . . . \n}`)
      ...
      for (let i = 0; i < TargetResult.length; i++) {
        ...
        await Promise.all(TargetResult[i].map(async(akun) => {
          if (!getFollowers.includes(akun.id) && akun.params.isPrivate === false) {
            ...
            console.log(chalk`[{magenta ${timeNow}}] {bold.red [SKIP]}${akun.params.username} => AKUN TARGET KE PRIVATE`)
          }
        }));
...
console.log(chalk`
  ...
  [?] {bold.green Auto Komen & Like Followers Target | Using Account/User Target!}
