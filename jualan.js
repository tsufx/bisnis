...
const CommentAndLike = async function(session, accountId, text){
  ...
  if (result.length > 0) {
    ...
    const printComment = Comment ? chalk`{green Komen}` : chalk`{red Komen}`;
...
const Excute = async function(User, TargetUsername, Text, Sleep, mysyntx){
  try {
    ...
    console.log(chalk`{green  [!] Login Succsess, }{yellow [?] Sukses Kita cek Id sama Followersnya . . .}`)
    ...
    console.log(chalk`{cyan  [?] Mencoba Jalankan Auto Komen dan Like Followers Target . . . \n}`)
